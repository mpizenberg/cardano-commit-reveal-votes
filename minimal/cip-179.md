...
```cddl
; CIP-179 On-chain Surveys and Polls (version 3)
;
; Encoding principles:
...
; Tag 0: Public.
;   Responses carry plaintext answer items (as in versions 1 and 2).
public_submission_mode = [0]

; Tag 1: Timelocked.
;   Responses carry a Drand `tlock` ciphertext instead of plaintext answers.
;   Answers are encrypted at submission and become decryptable by anyone once
;   `round` publishes on the pinned Drand chain. This is delayed reveal, not
;   permanent secrecy. `padding_size` is the minimum plaintext byte length each
;   response is padded to before encryption, so ciphertext length does not leak
;   answer content.
...
survey_definition = [
  uint,                        ; specVersion (this document = 3)
  ...
  submission_mode,                 ; how responses are submitted
...
; specVersion lets a response be decoded (including its answer encoding and
...
survey_response = [
  ...
  response_answers             ; answers (at least one; partial allowed)
]
...
|:---------|:-----|:------------|
| 0 | uint | Schema version. This document defines version `3`. |
...
```
[specVersion, survey_ref, role, credential, answers]
```
...
The response carries its own `specVersion`. A response's `specVersion` MUST match that of the referenced survey definition. Including it lets tooling decode a response — in particular its answer encoding and whether answers are plaintext or timelocked ciphertext — without first resolving the survey.
...
Version 3 extends version 2 with the following breaking changes. A `submission_mode` field is added to `survey_definition` as a tagged sum type (`[0]` public, `[1, chain_hash, round, padding_size]` timelocked), enabling timelocked responses: responses are submitted as Drand `tlock` ciphertext and become decryptable by anyone once the pinned round publishes (delayed reveal, not permanent secrecy). The `survey_definition` array is reordered so the fixed-size header fields precede the variable-length `questions` array, which now appears last: `[specVersion, owner, title, description, roleWeighting, endEpoch, submissionMode, questions]`. Question indices are unchanged, so `survey_response` answer references are unaffected. The `survey_response` answers field accepts either plaintext answer items (public surveys) or a chunked-bytes tlock ciphertext (timelocked surveys). A `specVersion` field is restored as the first element of `survey_response` (it was present in version 1, then dropped in version 2 on the assumption the referenced survey supplied it); without it, a response — including whether its answers are plaintext or ciphertext — cannot be decoded without first resolving the survey definition.
